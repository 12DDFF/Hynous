            ...
            rx.vstack(
                rx.foreach(
                    ...
                    lambda pos: rx.hstack(
                        ticker_badge(pos.symbol, font_size="0.85rem"),
                        rx.text(
                            pos.side.upper(),
...
def _wallet_detail() -> rx.Component:
    ...
    return rx.vstack(
        # LLM API — total + per-model breakdown
        rx.hstack(
            rx.box(
            ...
            rx.vstack(
                rx.text("LLM API", font_size="0.85rem", font_weight="500", color="#e5e5e5"),
                rx.hstack(
                    rx.text(AppState.wallet_llm_cost, font_size="0.8rem", color="#a78bfa", font_weight="500"),
                    rx.text("\u00b7", color="#333"),
                    rx.text(AppState.wallet_llm_calls + " calls", font_size="0.75rem", color="#525252"),
                    spacing="2",
                    align="center",
                ),
                rx.text(AppState.wallet_llm_tokens, font_size="0.7rem", color="#404040"),
                spacing="0",
            ),
            spacing="3",
            align="start",
        ...
        # Dynamic per-model breakdown (pre-rendered HTML)
        rx.box(
...
def profile_card() -> rx.Component:
    ...
    return rx.box(
        rx.vstack(
            ...
            rx.hstack(
                _stat_with_dialog("13", "Tools", "Available Tools", _tools_detail()),
...
def position_row(pos) -> rx.Component:
    ...
    return rx.hstack(
        # Symbol + Side
        rx.hstack(
            ticker_badge(pos.symbol, font_size="0.85rem", font_weight="500"),
            rx.text(
                pos.side.upper(),
                color=rx.cond(pos.side == "long", "#22c55e", "#ef4444"),
                font_size="0.7rem",
