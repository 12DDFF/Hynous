...
- **v2-debug audit — 15 of 18 issues resolved (2026-04-22)** — full status in **`docs/revisions/v2-debug/README.md`** with a Status Dashboard at the top. The 2026-04-21 v3 direction-model production outage (C1) was resolved on 2026-04-22: diagnose script on VPS confirmed v3's prediction distribution (p75 ≈ 1.7%, p95 ≈ 2.6%) is too narrow for the legacy 3.0% entry threshold. Fix landed as a one-line config change (`inference_entry_threshold: 3.0→2.0`, `inference_conflict_margin: 1.0→0.5`). H6 + H8 + M2 Option A landed in the same session. Remaining: M1 (held for a repurpose-to-backtesting rescope), M7, M9 (multi-PR refactors, deferred). No operational blockers.
...
*Last updated: 2026-04-22 (v3 production outage resolved via threshold calibration; 15 of 18 audit issues closed; M1 held for backtesting-repurpose brainstorm)*
