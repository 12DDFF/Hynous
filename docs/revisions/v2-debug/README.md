...
|----|----------|--------|-----------------|
| **C1** | Critical | **RESOLVED (two fixes)** | **Fix 1 (commit `aacb3a3`):** `inference_entry_threshold: 3.0→2.0`, `inference_conflict_margin: 1.0→0.5`. Needed because v3's narrower distribution (p99 ≈ 3.4% vs v2's ±20% clip range) made the legacy 3.0% threshold unreachable. **Fix 2 (commit `b09aed6`):** the direction-confidence gate in `ml_signal_driven.py:208` used `max(abs(long_roe), abs(short_roe)) / 10.0 >= 0.55`, which required max_roe ≥ 5.5% — p99.5+ on v3, so even trades that cleared gate 2 would reject at gate 3 with `direction_confidence_below_threshold`. Normalizer switched to `/5.0` with `min(1.0, ...)` clamp so the same 0.55 threshold now means max_roe ≥ 2.75 (≈ p95), and downstream sizing tiers (0.6 / 0.8 → tier_medium / tier_high) map to empirically-distinct strength buckets instead of collapsing to tier_speculative. Together the fixes produce a system that will actually fire trades on typical v3 signals while preserving conviction-based sizing differentiation. |
