...
├── consolidation.py      # Background consolidation engine (cross-episode pattern → lesson/playbook)
├── playbook_matcher.py   # Matches scanner anomalies against stored playbook triggers
...
│
└── tools/                # Tool definitions (21 modules, 28 tools — see tools/README.md)
    ...
    └── pruning.py        # analyze_memory, batch_prune
```
...
- **`docs/archive/nous-wiring/executive-summary.md`** -- Overview of all issue categories
- **`docs/archive/revision-exploration.md`** -- Master issue list (19 issues, P0-P3)
...
Post-MF feature: **Intelligent Retrieval Orchestrator** -- `retrieval_orchestrator.py` implements multi-pass memory search with compound query decomposition, quality gating, and dynamic result sizing. Uses `NousClient.search_full()` and `classify_query()`. See `docs/archive/memory-search/` for design docs.

### Trade Recall -- RESOLVED

Three trade retrieval bugs have been fixed. See `docs/archive/trade-recall/retrieval-issues.md` for root cause analysis and resolution details.
