...
- **C1 (two fixes)** — Fix 1 (`aacb3a3`): `inference_entry_threshold 3.0→2.0`, `inference_conflict_margin 1.0→0.5`. Fix 2 (`b09aed6`): direction-confidence normalizer `/10.0 → /5.0` in `ml_signal_driven.py:208` with `min(1.0, ...)` clamp, because the v1-era `/10` divisor made gate 3 require max_roe ≥ 5.5% (blue moon on v3). With `/5` the same 0.55 threshold maps to max_roe ≥ 2.75 (≈p95 on v3), and downstream sizing tiers (0.6 / 0.8) map to max_roe ≥ 3.0 / ≥ 4.0 — preserving tier differentiation on v3's narrower distribution. Both fixes deployed + verified live.
