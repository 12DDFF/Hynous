...
| 4 | Tier 1 deletions | complete | 2026-04-12 | 9 (M1–M9) | Nous server + Python Nous client + 9 decision-injection modules + 8 memory tools + unused coinglass methods + orphan tests + prompt trim + daemon health removal. Post-phase-4 baseline: `pytest tests/` = 482p/0f, registry = 17 tools. Deferred to phase 7: `trade_analytics.py`, `memory_tracker.py`, dashboard memory/graph/brain pages, Makefile/pyproject/deploy rework, cryptocompare + perplexity + news alert detector, CRYPTOCOMPARE_API_KEY. |
| 5 | Mechanical entry | complete | 2026-04-12 | 8 (M1–M8) | `src/hynous/mechanical_entry/` (interface + `MLSignalDrivenTrigger` + `compute_entry_params` + `executor`), daemon rewire (`_evaluate_entry_signals` + periodic 60 s `_periodic_ml_signal_check`), v1 `intelligence/agent.py` + `handle_execute_trade` deleted, `execute_trade` unregistered, user chat agent at `src/hynous/user_chat/` with `/api/v2/chat/*`. Final baselines: 551p/0f, mypy 252, ruff 62. |
| 6 | Consolidation & patterns | complete | 2026-04-12 | 5 (M1–M5) | `src/hynous/journal/consolidation.py` — 4 edge builders (temporal preceded/followed-by, regime-bucket 30 d/10-cap, rejection-reason grouping, rejection-vs-contemporaneous 2 h window) + `run_weekly_rollup` writing `system_health_report` patterns with mistake_tag_summary/rejection_reasons/grade_summary/regime_performance. Edge builds fire automatically after analysis insert via `build_edges_for_trade`; daemon starts `start_weekly_rollup_cron` (interval from `V2Config.consolidation.pattern_rollup_interval_hours`). Routes `/api/v2/journal/patterns` and `/api/v2/journal/trades/{id}/related`. CLI manual trigger `python -m hynous.journal rollup`. No-LLM guarantee preserved (grep `litellm\|completion` in `consolidation.py` → 0). Final baselines: 576p/0f, mypy 252, ruff 62, registry 18. |
| 7 | Dashboard rework | complete | 2026-04-12 | 9 (M1–M9) | Memory + Graph pages and `brain.html` + `graph.html` deleted; `/api/nous` proxy removed. Journal page rewritten against `/api/v2/journal/*` with 6 fetchers (trades list, trade detail, related, stats, patterns, semantic search). `intelligence/agent.py`, `memory_tracker.py`, `trade_analytics.py`, `market_watch`/`trade_stats`/`watchpoints` tools, `DiscordConfig` + `src/hynous/discord/`, cryptocompare + perplexity providers + news alert detector all removed. `briefing.py` prompt sections trimmed 435→244 lines. Makefile refreshed, systemd services pruned 3→2. Nav shows 7 items (Home/Chat/Journal/Data/ML/Settings/Debug; Login is pre-auth, not in nav bar — minor drift from plan's literal "8 items"). state.py: 3947 → 2785 lines. Three component READMEs rewritten to match v2 reality (intelligence, core, data). Final baselines: **576p/0f, mypy 223, ruff src 51, ruff dashboard 120, registry 15**. Smoke: `storage/v2/smoke-phase-7.log`. See `v2-planning/phase-7-acceptance.md`. |
| 8 | Quantitative improvements | complete | 2026-04-13 | 7 (pre-landed f573dd8/4e6beb0 + new-M1..M5) | Task 1 tick downsample + retrain (`f573dd8` — 5 s downsample in `tick_inference.py:232-237`; 8 tick models in `satellite/artifacts/tick_models/direction_{10..180}s/`, 53–67 % dir accuracy). Task 2 MC fixes (`4e6beb0` — feature-list consolidation + `_zero_count >= 10` guard + bias-score strong-only). Task 3 composite-score calibration audit script (`f3b01ee`, new-M3). Task 4 weight-update tightening — `min_trades 30→10` + EMA smoothing + daily interval (`3e84c55`, new-M1) + seeded MC RNG covering tick-audit Issue 5 (`56eee0d`, new-M2). Task 5 direction-model retrain bridge (`514db5a`, new-M4). Final baselines: **592p/0f, mypy 223/40, ruff src 51, ruff dashboard 120, registry 15**. Smoke log: `storage/v2/smoke-phase-8.log` (30-min paper, zero exceptions). See `v2-planning/phase-8-acceptance.md`. |
