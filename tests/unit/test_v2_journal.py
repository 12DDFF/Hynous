"""Phase 2 Milestone 1 — dataclass reconstruction helpers (Amendment 9).

These tests verify that :func:`entry_snapshot_from_dict` and
:func:`exit_snapshot_from_dict` round-trip every field of the snapshot
dataclasses through ``asdict → json.dumps → json.loads → from_dict``. The
end-to-end ``test_get_trade_hydrates_nested_dataclasses`` is stubbed and
intentionally skipped until :class:`JournalStore` lands in Milestone 2 —
this is the ONLY phase-2 test allowed to use ``@pytest.mark.skip`` and it
is documented in the architect's Milestone 1 plan as such.
"""
...
# ---------------------------------------------------------------------------
# Fixtures — fully populated snapshot instances so the round-trip equality
# check exercises every field.
# ---------------------------------------------------------------------------


@pytest.fixture
def sample_entry_snapshot() -> TradeEntrySnapshot:
    """A TradeEntrySnapshot with every field set to a plausible non-None value.

    Non-None coverage is deliberate — if any field were left at its default
    None, a bug in the reconstruction helper that swapped or dropped that
    field would be invisible to ``==``. Every field being populated makes
    the round-trip test meaningfully exhaustive.
    """
...
@pytest.fixture
def sample_exit_snapshot() -> TradeExitSnapshot:
    """A TradeExitSnapshot with every field set to a plausible non-None value."""
...
def test_exit_snapshot_round_trip_preserves_every_field(
    ...
    """Same round-trip contract for TradeExitSnapshot."""
...
# NOTE: the following test is the only @pytest.mark.skip in phase 2. It's the
# end-to-end JournalStore hydration test; JournalStore itself lands in
# Milestone 2, so the test body is written now but skipped until then. This
# exception is explicit in the architect's Milestone 1 plan (Amendment 9
# follow-through). Un-skip in M2.
@pytest.mark.skip(reason="JournalStore lands in Milestone 2")
def test_get_trade_hydrates_nested_dataclasses(
    tmp_path, sample_entry_snapshot: TradeEntrySnapshot
) -> None:
    """End-to-end: insert → get_trade → returned bundle has real dataclass
    instances at entry_snapshot/exit_snapshot keys, not raw dicts."""
    from hynous.journal.store import JournalStore  # type: ignore[import-not-found]

    store = JournalStore(str(tmp_path / "journal.db"))
    try:
        store.insert_entry_snapshot(sample_entry_snapshot)
        bundle = store.get_trade(sample_entry_snapshot.trade_basics.trade_id)
